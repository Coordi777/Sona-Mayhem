    ...
    configureServer(server) {
      server.httpServer?.on('listening', () => {
        ...
        const devEntry = [
          ...
          `}).catch(e => {`,
          `  console.error("[Sona-Mayhem] Failed to install react preamble:", e);`,
          `  throw e;`,
          `});`,
          `const viteClient = reactPreamble.then(() => import("https://localhost:${port}/@vite/client")).catch(e => {`,
          `  console.error("[Sona-Mayhem] Failed to load vite client:", e);`,
          `  throw e;`,
          `});`,
          `const pluginModule = () => viteClient.then(() => import("https://localhost:${port}/src/index.tsx")).catch(e => {`,
          `  console.error("[Sona-Mayhem] Failed to load plugin module:", e);`,
          `  throw e;`,
          `});`,
          ``,
          `export function init(context) {`,
          `//  console.log("[Sona-Mayhem] init called");`,
          `  pluginModule().then(m => m.init(context)).catch(e => console.error("[Sona-Mayhem] init error:", e));`,
          `}`,
          ``,
          `export function load() {`,
          `//  console.log("[Sona-Mayhem] load called");`,
          `  pluginModule().then(m => m.load?.()).catch(e => console.error("[Sona-Mayhem] load error:", e));`,
          `}`,
        ].join('\n')
    ...
    closeBundle() {
      ...
      if (fs.existsSync(jsPath)) {
        ...
        const banner = [
          ...
          ' * @name Sona-Mayhem',
          ` * @version ${pkg.version}`,
          ' * @description Sona 的海克斯大乱斗增强 fork — 在原 Sona 基础上新增战利品助手、大乱斗智能配装、海克斯 augment 速查等功能',
          ' * @author Coordi777 (forked from WJZ_P)',
          ' * @link https://github.com/Coordi777/sona-mayhem',
          ' */',
          '',
        ].join('\n')
        js = banner + js
